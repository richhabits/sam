...
export const TOOLS: Tool[] = [
  ...
  { name: "set_timer", safe: true, description: "Set a timer that pops a notification when done. input: {minutes, label?}.", params: "{minutes, label?}",
    activity: (i) => `Setting a ${i.minutes ?? 5}-min timer`,
    run: async (i) => { if (!IS_MAC) return needsMac("Timers"); const m = Math.max(1, Math.min(600, Number(i?.minutes) || 5)); const label = String(i?.label || "Timer done").replace(/["'\\]/g, ""); sh(`(sleep ${m * 60} && osascript -e 'display notification "${label}" with title "⏰ SAM Timer" sound name "Glass"') >/dev/null 2>&1 &`).catch(() => {}); return `⏰ Timer set for ${m} min. I'll ping you.`; } },
  { name: "password_generate", safe: true, description: "Generate a strong random password. input: {length?}.", params: "{length?}",
    activity: () => `Generating a strong password`,
    run: async (i) => { const n = Math.max(8, Math.min(64, Number(i?.length) || 20)); const { stdout } = await sh(`LC_ALL=C tr -dc 'A-Za-z0-9@#%^&*_-' < /dev/urandom | head -c ${n}`); return `🔑 ${stdout}\n(Copy it now — I never store passwords.)`; } },
  ...
  { name: "system_info", safe: true, description: "Quick system snapshot: CPU, RAM, uptime & load.", params: "(none)",
    activity: () => `Checking system stats`,
    run: async () => { try { const cpu = (await sh(`sysctl -n machdep.cpu.brand_string 2>/dev/null || uname -p`)).stdout.trim(); const mem = Number((await sh(`sysctl -n hw.memsize 2>/dev/null || echo 0`)).stdout.trim()); const up = (await sh(`uptime | sed 's/.*up //; s/, [0-9]* user.*load/ · load/'`)).stdout.trim(); return `💻 ${cpu}\n  RAM: ${Math.round(mem / 1e9)}GB · ${up}`; } catch (e: any) { return `Couldn't read system info: ${e?.message}`; } } },
  ...
  { name: "wifi_status", safe: true, description: "Which Wi-Fi network am I on.", params: "(none)",
    activity: () => `Checking Wi-Fi`,
    run: async () => { if (!IS_MAC) return needsMac("Wi-Fi info"); const { stdout } = await sh(`networksetup -getairportnetwork en0 2>/dev/null || echo "unknown"`); return `📶 ${stdout.trim()}`; } },
  { name: "set_volume", safe: true, description: "Set the Mac's output volume 0-100. input: {level}.", params: "{level}",
    activity: (i) => `Setting volume to ${i.level ?? i}%`,
    run: async (i) => { if (!IS_MAC) return needsMac("Volume"); const v = Math.max(0, Math.min(100, Number(i?.level ?? i) || 50)); await sh(`osascript -e 'set volume output volume ${v}'`); return `🔊 Volume set to ${v}%`; } },
  { name: "lock_screen", safe: false, description: "Lock the Mac screen now.", params: "(none)",
    activity: () => `Locking your screen`, preview: () => `Lock the Mac screen now`,
    run: async () => { if (!IS_MAC) return needsMac("Lock screen"); await sh(`pmset displaysleepnow`); return `🔒 Screen locked.`; } },
  { name: "caffeinate", safe: true, description: "Keep the Mac awake for N minutes. input: {minutes?}.", params: "{minutes?}",
    activity: () => `Keeping your Mac awake`,
    run: async (i) => { if (!IS_MAC) return needsMac("Caffeinate"); const m = Math.max(1, Math.min(480, Number(i?.minutes) || 30)); sh(`caffeinate -di -t ${m * 60} >/dev/null 2>&1 &`).catch(() => {}); return `☕ Keeping your Mac awake for ${m} min.`; } },
  ...
  { name: "currency_convert", safe: true, description: "Convert currency. input: {amount, from, to} — e.g. 100 USD GBP.", params: "{amount, from, to}",
    activity: (i) => `Converting ${i.amount} ${i.from} → ${i.to}`,
    run: async (i) => { try { const from = String(i.from).toUpperCase(), to = String(i.to).toUpperCase(), amt = Number(i.amount) || 1; const d: any = await (await fetch(`https://open.er-api.com/v6/latest/${from}`)).json(); const rate = d?.rates?.[to]; return rate ? `💱 ${amt} ${from} = ${(amt * rate).toFixed(2)} ${to}  (rate ${rate})` : `Couldn't get ${from}→${to}.`; } catch (e: any) { return `Currency lookup failed: ${e?.message}`; } } },
