...
export function friendlyUpdateError(raw: string): string {
  ...
  if (/diverged|non-fast-forward|would be overwritten|Not possible to fast-forward/i.test(msg)) return "Your copy has diverged from GitHub. Run `git pull` in the sam folder to bring it back in line, or reinstall the app.";
