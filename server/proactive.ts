...
export function desktopNotify(title: string, msg: string) {
  ...
  if (process.platform === "darwin") {
    execFile("osascript", ["-e", `tell application "SAM" to display notification "${e(clean)}" with title "${t}"`], () => {/* fire-and-forget: a notification that fails must not affect the caller */});
