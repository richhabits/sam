...
describe("ingestFolder", () => {
  ...
  it("respects the per-run file cap and reports the remainder", async () => {
    for (let i = 0; i < 4; i++) writeFileSync(join(docsDir, `f${i}.md`), `Document number ${i} — long enough content about the hectic radio invoice supplier world to index properly.`);
