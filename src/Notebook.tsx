                ...
                <div className="nb-chat" ref={chatRef}>
                  ...
                  {turns.map((t, i) => (
                    <div key={i} className="nb-turn">
                      ...
                      <div className="nb-a">
                        {t.loading ? <span className="nb-typing">consulting your sources…</span> : <div dangerouslySetInnerHTML={{ __html: renderMarkdown(t.a) }} />}
                        {t.citations.length > 0 && <div className="nb-cites">📎 {t.citations.slice(0, 6).join(" · ")}</div>}
                      </div>
                    </div>
                  ))}
                </div>
