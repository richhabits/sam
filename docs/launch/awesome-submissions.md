...
> accept free/source-available tools. **Decision for the user:** relicense to open-source to unlock the big
...
```md
- [SAM](https://github.com/richhabits/sam) - Free, private, local-first AI assistant with a team-of-agents mode and 167 tools; runs on rotating free tiers or fully offline on Ollama.
```

## 2. awesome-local-llm / awesome-local-ai  (janhq/awesome-local-ai) — ✅ good fit
Rules: tools that run LLMs locally; note the local capability clearly. Entry:
```md
- [SAM](https://github.com/richhabits/sam) - Local-first AI assistant; defaults to your Ollama model when you have no cloud keys (private + offline), with 167 tools and an agent team.
```

## 3. awesome-privacy  (pluja/awesome-privacy) — ✅ good fit (accepts source-available)
Rules: privacy-respecting tools; state the privacy property. Entry:
```md
...
## Submission tips
- Read each list's CONTRIBUTING before opening the PR; match its exact format/section.
- One clean commit, alphabetical placement, no marketing adjectives in the description.
- Link to the repo, not the landing page (lists prefer source).
