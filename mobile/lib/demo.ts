...
const JOBS = [
  ...
  // The rest of the kinds server/yard/worker.ts can dispatch. Present so the demo shows what
  // SAM actually does rather than four variations on one thing — and so every branch of
  // taskGlyph is exercised by something a person can look at. Four of those marks had never
  // been rendered on a screen before this.
  {
    id: 'demo_job_build',
    kind: 'project.build',
    payload: { name: 'Teahouse', slug: 'teahouse', what: 'a one-page site for the shop' },
    state: 'done' as const,
    createdAt: ago(11),
    startedAt: ago(11),
    finishedAt: ago(11) + 96_000,
    steps: [
      { label: 'Created the project', state: 'done' as const, at: ago(11) },
      { label: 'Built the site', state: 'done' as const, at: ago(11) },
      { label: 'Ran the checks', state: 'done' as const, at: ago(11) },
    ],
    costTokens: 2_400,
    project: 'teahouse',
    lastError: null,
  },
  {
    id: 'demo_job_create',
    kind: 'project.create',
    payload: { name: 'Invoices', what: 'somewhere to keep the quarterly numbers' },
    state: 'done' as const,
    createdAt: ago(14),
    startedAt: ago(14),
    finishedAt: ago(14) + 8_000,
    steps: [{ label: 'Created invoices', state: 'done' as const, at: ago(14) }],
    costTokens: 120,
    project: 'invoices',
    lastError: null,
  },
  {
    id: 'demo_job_edit',
    kind: 'project.edit',
    payload: { slug: 'teahouse', what: 'change the opening hours' },
    state: 'done' as const,
    createdAt: ago(16),
    startedAt: ago(16),
    finishedAt: ago(16) + 19_000,
    steps: [
      { label: 'Read content/hours.md', state: 'done' as const, at: ago(16) },
      { label: 'Edited content/hours.md', state: 'done' as const, at: ago(16) },
    ],
    costTokens: 340,
    project: 'teahouse',
    lastError: null,
  ...
  {
    id: 'demo_job_checkpoint',
    kind: 'project.checkpoint',
    payload: { slug: 'teahouse', what: 'before the hours change' },
    state: 'done' as const,
    createdAt: ago(17),
    startedAt: ago(17),
    finishedAt: ago(17) + 3_000,
    steps: [{ label: 'Saved a checkpoint', state: 'done' as const, at: ago(17) }],
    costTokens: 0,
    project: 'teahouse',
    lastError: null,
  },
  {
    ...
    payload: { slug: 'teahouse', what: 'put the old hours back' },
    state: 'done' as const,
    createdAt: ago(18),
    startedAt: ago(18),
    finishedAt: ago(18) + 4_000,
    steps: [{ label: 'Restored the checkpoint', state: 'done' as const, at: ago(18) }],
    costTokens: 0,
    project: 'teahouse',
    lastError: null,
  },
  {
    id: 'demo_job_run',
    kind: 'run',
    payload: { slug: 'teahouse', what: 'install and test' },
    state: 'done' as const,
    createdAt: ago(20),
    startedAt: ago(20),
    finishedAt: ago(20) + 61_000,
    steps: [
      { label: 'Installed dependencies', state: 'done' as const, at: ago(20) },
      { label: 'Ran the tests', state: 'done' as const, at: ago(20) },
    ],
    costTokens: 0,
    project: 'teahouse',
    lastError: null,
  ...
  {
    id: 'demo_job_playbook',
    kind: 'playbook.run',
    payload: { name: 'Morning check', what: 'the usual sweep' },
  ...
  },
  {
    ...
    createdAt: ago(22),
    startedAt: ago(22),
    finishedAt: ago(22) + 2_000,
    steps: [{ label: 'sleeping 5s', state: 'stopped' as const, at: ago(22) }],
    costTokens: 0,
    project: null,
    lastError: null,
  },
...
// COUNTED, NOT DECLARED. These were six hardcoded numbers sitting beside the list they claimed
// to describe, so adding a job made the "Now" section quietly lie — the same defect the desk
// audit found in its own status subtitles. A summary derived from the data cannot drift from it.
